...
public class OkHttpTool {
    public static String URL ="http://117.72.14.57:9782";
    //日志标志
    ...
    static {
        ...
        myOkHttpClient = new OkHttpClient.Builder()
                .connectTimeout(50000, TimeUnit.SECONDS)//连接超时
                .writeTimeout(50000, TimeUnit.SECONDS)//写入超时
                .readTimeout(50000, TimeUnit.SECONDS)//读取超时
    ...
    @SuppressWarnings("unused")
    public static void httpNoAuthorizationGet(String url, Map<String, Object> parameters, ResponseCallback responseCallback) {
        Request request = createNoAuthorizationGetRequest(url, parameters);
        doRequest(request, responseCallback);
    }
    ...
    public static void httpRegisterPostJson(String url, String json, ResponseCallback responseCallback) {
        Request request = createNoAuthorizedPostRequestJson(url, json);
        doRequest(request, responseCallback);
    }

    public static void httpDeleteJson(String url, String json, ResponseCallback responseCallback) {
        Request request = createDeleteRequestJson(url, json);
        doRequest(request, responseCallback);
    }
    ...
    private static Request createGetRequest(String url, Map<String, Object> parameters) {
        ...
        return getBaseRequest(true).url(urlBuilder.toString()).build();
    }

    //构建没有 Authorization 的 Get请求
    private static Request createNoAuthorizationGetRequest(String url, Map<String, Object> parameters) {
        StringBuilder urlBuilder = new StringBuilder();
        urlBuilder.append(url);
        if (url.indexOf('?') <= -1) {
            //未拼接参数
        ...
        return getBaseRequest(false).url(urlBuilder.toString()).build();
    }
    ...
    private static Request createPostRequest(String url, Map<String, Object> parameters) {
        ...
        return getBaseRequest(true).url(url).post(formBody).build();
    }
    ...
    private static Request createPostRequestJson(String url, String json) {
        ...
        return getBaseRequest(true).url(url).post(body).build();
    }

    //构建注册的 POST JSON格式参数请求
    private static Request createNoAuthorizedPostRequestJson(String url, String json) {
        MediaType mediaType = MediaType.parse("application/json;charset=utf-8");
        RequestBody body = RequestBody.create(json,mediaType);
        return getBaseRequest(false)  // 对于注册，不添加 Authorization 头
                .url(url)
                .post(body)
    ...
    private static Request createDeleteRequestJson(String url, String json) {
        MediaType mediaType = MediaType.parse("application/json;charset=utf-8");
        RequestBody body = RequestBody.create(json,mediaType);
        return getBaseRequest(true).url(url).delete(body).build();
    }
    ...
    private static Request createPostRequestWithFile(String url, Map<String, Object> parameters, Map<String, File> files) {
        ...
        return getBaseRequest(true).url(url).post(requestBody.build()).build();
    }
    ...
    private static Request.Builder getBaseRequest(boolean includeAuth) {
        Request.Builder builder = new Request.Builder();
        //builder.addHeader("client", "Android");
        //builder.addHeader("Connection", "keep-alive");
//        builder.addHeader("Authorization","Bearer "+ OpenActivity.access);
//        return builder;
        if (includeAuth && OpenActivity.access != null && !OpenActivity.access.isEmpty()) {
            builder.addHeader("Authorization", "Bearer " + OpenActivity.access);
        }
        return builder;
    }
